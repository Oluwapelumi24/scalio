...
function DeleteBtn({ id }: { id: string }) {
  ...
  return (
    <button
      disabled={pending}
      onClick={() => { if (!confirm('Delete this item?')) return; startTransition(async () => { await deleteService(id) }) }}
...
export function ItemList({ items, emptyLabel }: { items: ServiceItem[]; emptyLabel: string }) {
  if (items.length === 0) {
    return (
